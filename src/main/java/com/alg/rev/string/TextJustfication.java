...
public class TextJustfication {
	...
	public static List<String> justfication(String[] words, int lineLen) {
		...
		if (words != null && words.length > 0) {
			...
			for (int i = 0; i < words.length; i++) {
				...
				if ((curLineLen + oneWord.length()) < lineLen) {
					sb.append(oneWord + " ");
					curLineLen += oneWord.length() + 1;
				} else {

					while (curLineLen < lineLen) {
						sb.append(" ");
						curLineLen++;
